...
def parse_date(s: str) -> date:
    ...
    try:
        ...
        y, m, d = map(int, parts)
        return date(y, m, d)  # will validate calendar (incl. leap years)
...
def serialize_value(db_type: str, value: Any) -> Any:
    ...
    if t == "dateinvl":
        ...
        # allow tuple/list of two date strings
        a, b = value
...
def coerce_value(db_type: str, raw: Any) -> Any:
    """
    Coerce user input (typically string) into internal typed value.
    """
    t = db_type.lower()
    if raw is None:
    ...
    if t == "dateinvl":
        # Accept: "YYYY-MM-DD..YYYY-MM-DD" OR tuple/list of two inputs
